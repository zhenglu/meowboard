...
int SeekGraphClick(ClickType click, int x, int y, int moving)
{
	...
	if(dist < 120) {
	    ...
	    SendToICS(buf);
#endif
        /* information to setup a challenge to others */
         setup_a_challenge(seekAdList[closest]);
...
//    SendToICS(user_move);
    send_xmpp_mymove_message(user_move);
     ...
     int redraw, init;
{
    ...
    gameMode = BeginningOfGame;
    ModeHighlight();
    /*FIXME: I need a xiangqi layout in the ICS mode */
    if(appData.icsActive) gameInfo.variant = VariantXiangqi;
...
void set_loggedOn(int status)
{
    loggedOn = status;
}
void set_soughtPending(int flag)
{
    soughtPending = flag;
}

void set_seekGraphUp(int flag)
{
    seekGraphUp = flag;
}
...
void set_ics_user_moved(int flag)
{
    ics_user_moved = flag;
}
int get_ics_user_moved()
{
    return ics_user_moved;
}
/***
ics_flip: should the board is flipped over? 0 - keep the previous style, 1 - flipped
          currently it is not used, it was set to 0 in this function.
ticking:
          currently it is not used, it was set to 2 in this function.


***/
void set_board_style(char to_play, int double_push,
                     ...
                     char *long_str, char *elapsed_time,
                     char *move_str, int ics_flip,
                     int ticking, char *xmpp_board_chars)
{
    GameMode newGameMode;
    int newGame, newMove, i;
    int j, k, n, takeback;
    Board board;
    int prevMove = currentMove;
    ChessMove moveType;
    int fromX, fromY, toX, toY;
    char promoChar;
    int ranks=10, files=9; /* [HGM] ICS80: allow variable board size */
    char *bookHit = NULL; // [HGM] book
    Boolean weird = FALSE, reqFlag = FALSE;
    char board_chars[200];

    convert_board_express(xmpp_board_chars,board_chars);

    ticking = 2;

    ics_flip = 0; /* do not flip the board style*/
    ...
    if((gameMode == IcsPlayingWhite || gameMode == IcsPlayingBlack)
	 && newGameMode == IcsObserving && gamenum != ics_gamenum && appData.bgObserve) {
      ...
      CopyBoard(partnerBoard, board);
      if(toSqr = strchr(long_str, '/')) { // extract highlights from long move
        partnerBoard[EP_STATUS-3] = toSqr[1] - AAA; // kludge: hide highlighting info in board
        partnerBoard[EP_STATUS-4] = toSqr[2] - ONE;
      } else partnerBoard[EP_STATUS-4] = partnerBoard[EP_STATUS-3] = -1;
      if(toSqr = strchr(long_str, '-')) {
        partnerBoard[EP_STATUS-1] = toSqr[1] - AAA;
        partnerBoard[EP_STATUS-2] = toSqr[2] - ONE;
      } else partnerBoard[EP_STATUS-1] = partnerBoard[EP_STATUS-2] = -1;
      if(appData.dualBoard && !twoBoards) { twoBoards = 1; InitDrawingSizes(-2,0); }
...
//   SendToICS(str);
  ...
  if (appData.debugMode) {
	...
	if (moveNum <= backwardMostMove) {
	  ...
	  // str looks something like "Q/a1-a2"; kill the slash
	  if(long_str[1] == '/')
	    snprintf(buf, MSG_SIZ,"%c%s", long_str[0], long_str+2);
	  else  safeStrCpy(buf, long_str, sizeof(buf)/sizeof(buf[0])); // might be castling
	  if((prom = strstr(move_str, "=")) && !strstr(buf, "="))
		strcat(buf, prom); // long move lacks promo specification!
	  if(!appData.testLegality && move_str[1] != '@') { // drops never ambiguous (parser chokes on long form!)
		if(appData.debugMode)
...
void  convert_board_express(char *xmpp_board_chars,char *board_chars)
{
    int i=0;
    int j=0;
    int xmpplen = strlen(xmpp_board_chars);
    for(i = 0; i<xmpplen;i++)
    {
        if(xmpp_board_chars[i]=='/')
        {
            board_chars[j] = ' ';
            j++;
        }
        else if(xmpp_board_chars[i]>'0' && xmpp_board_chars[i] <= '9')
        {
            int k = xmpp_board_chars[i]-'0';
            int n = 0;
            for(;n<k;n++)
            {
                board_chars[j] = '-';
        ...
        {
            board_chars[j]=xmpp_board_chars[i];
            j++;
        }
    }
    board_chars[j]='\0';
...
void set_gs_gamenum(int gamenum)
{
    gs_gamenum = gamenum;
}

/* "xiangqi" is one of the VARIANT_NAMES*/
void set_gs_kind(char * why)
{
    safeStrCpy(gs_kind, why,sizeof(gs_kind)/sizeof(gs_kind[0]));
}

char * get_xmpp_hostname()
{
    return appData.icsHost;
}
