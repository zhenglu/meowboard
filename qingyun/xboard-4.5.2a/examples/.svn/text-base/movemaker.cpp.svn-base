    ...
    for (index = 0x33; index <= 0xcb; index++)
    {
        ...
        switch (cur.position[index])
        {
            ...
            case B_CANNON:
                if (who == GetColor(cur.position[index]))
                {
                    for (next = 0, tar = index - 0x10; tar > 0x30; tar -= 0x10)
                    {
                        if ((next == 0) && (cur.position[tar] == NONE))
                        {
                            if (PreAdd(cur, who, index, tar, chk))
                            {
                                Movement mv(index, tar, cur.position[tar], chk);
                        ...
                        if ((next == 1) && ((who == BLACK) ? IsRed(cur.position[tar]) : IsBlack(cur.position[tar])))
                        {
                            if (PreAdd(cur, who, index, tar, chk))
                            {
                                Movement mv(index, tar, cur.position[tar], chk);
                        ...
                        if ((next == 1) && ((who == BLACK) ? IsBlack(cur.position[tar]) : IsRed(cur.position[tar])))
                        {
                            break;
                        }
                    }
                    for (next = 0, tar = index + 0x10; tar < 0xd0; tar += 0x10)
                    {
                        if ((next == 0) && (cur.position[tar] == NONE))
                        {
                            if (PreAdd(cur, who, index, tar, chk))
                    ...
                    for (next = 0, tar = index - 0x01; (tar & 0x0f) > 0x02; tar -= 0x01)
                    {
                        if ((next == 0) && (cur.position[tar] == NONE))
                        {
                            if (PreAdd(cur, who, index, tar, chk))
                    ...
                    for (next = 0, tar = index + 0x01; (tar & 0x0f) < 0x0c; tar += 0x01)
                    {
                        if ((next == 0) && (cur.position[tar] == NONE))
                        {
                            if (PreAdd(cur, who, index, tar, chk))
...
int MoveMaker::PreAdd(Board & cur, PLAYER who, BDPOINT from, BDPOINT to, int & check)
{
    check = 0;
    
    int i, flag;
    int face = 1, suicide = 0;
    BDPOINT tar, leg;
