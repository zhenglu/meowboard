...
class ClientSample : public Client
{
  public:
    ...
    {
        JID componentjid( "www.xiaxiaqi.com" );
        IQ iq(IQ::Get, componentjid,"auth_1");
        Query *q = new Query( "robottest" );
        iq.addExtension( q );
        send(iq);
    ...
    virtual bool handleNormalNode( Tag* tag )
    {
        ...
        {
//            printf("current status=2,tag=%s\n",tag->name());
            if( tag->xmlns().empty() || tag->xmlns() == XMLNS_CLIENT )
            {
              if( tag->name() == "iq"  )
              {
                  if( tag->xml() != "<iq type='result' from='www.xiaxiaqi.com' id='auth_1' xmlns='jabber:client'><query xmlns='jabber:iq:auth'><username>robottest</username><password/><digest/><resource/></query></iq>" )
                  {
//                    printf("status=1\n");
                  }
                  else
                  {
                    ...
                    new Tag( q, "resource" );
                    Query n( q );
                    Query* nq = n.newInstance( "robottest", "sid", "robottest", "ChessD" );
        ...
        else if(3 == m_current_status) //I'm now online
        {
            if( tag->xmlns().empty() || tag->xmlns() == XMLNS_CLIENT )
            {
              if( tag->name() == "iq"  )
              {
                  if( tag->xml() == "<iq type='result' from='www.xiaxiaqi.com' id='auth_2' xmlns='jabber:client'/>" )
                  {

                    JID componentjid( "chessd.www.xiaxiaqi.com" );
                    ...
                    JID generaljid( "general@conference.www.xiaxiaqi.com/robottest" );
                     ...
                     if(query)
                     {
                        if(query->xmlns() == XMLNS_XQ_OFFER)
                        {
                            ...
                            if(match)
                            {
                                ...
                                std::string xml_header = "<iq xml:lang='zh-cn' type='set' to='chessd.www.xiaxiaqi.com' id='Challenge'><query xmlns='http://c3sl.ufpr.br/chessd#match#accept'><match id='";
                    ...
                    MessageEventHandler, MessageHandler, ChatStateHandler
{
  public:
    ...
    void start()
    {
      ...
      JID jid( "robottest@www.xiaxiaqi.com/ChessD" );
//password,
//port:?
      j = new ClientSample( jid, "robottest" );
      ...
      ConnectionTCPClient* conn0 = new ConnectionTCPClient( j->logInstance(), "192.168.250.9", 8082 );
//      ConnectionHTTPProxy* conn1 = new ConnectionHTTPProxy( conn0, j->logInstance(), "www.xiaxiaqi.com", 8082 );
      ConnectionBOSH* conn2 = new ConnectionBOSH( j, conn0, j->logInstance(), "192.168.250.9", "192.168.250.9" );
...
//      ConnectionTCPClient* conn0 = new ConnectionTCPClient( j->logInstance(), "www.xiaxiaqi.com", 80 );
 //     ConnectionBOSH* conn1 = new ConnectionBOSH( j, conn0, j->logInstance(), "www.xiaxiaqi.com", "www.xiaxiaqi.com" );
...
//			XMPP = "<presence to='"+RoomName+"/"+MainData.Username+"' />";
		...
		{
			XMPP = "<presence to='"+RoomName+"/"+"robottest"+"' ><show>"+NewStatus+"</show></presence>";
		}
	}
...
//                          printf("substr=%s",ai_move.substr(0,9).c_str());
    ...
    else if(ai_move.substr(0,9)== "bestmove ")
    {
        ...
        Buffer += GameID;
        Buffer += "@chessd.www.xiaxiaqi.com' id='GameMove'><query xmlns='http://c3sl.ufpr.br/chessd#game#move'><move long='";
