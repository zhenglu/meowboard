...
namespace gloox
{
  ...
  ConnBoshMultStat::ConnBoshMultStat( ConnectionBase* connection, const LogSink& logInstance,
  ...
  void ConnBoshMultStat::initInstance( ConnectionBase* connection, const std::string& xmppServer,
                                     const int xmppPort )
  {
// FIXME: check return value
    local_idna( xmppServer, m_server );
    m_port = xmppPort;
    if( m_port != -1 )
    {
  ...
  ConnBoshMultStat::~ConnBoshMultStat()
  {
    util::clearList( m_activeConnections );
    util::clearList( m_connectionPool );
  }

  ConnectionBase* ConnBoshMultStat::newInstance() const
  {
    ...
    return new ConnBoshMultStat( m_handler, pBaseConn, m_logInstance,
                               m_boshHost, m_server, m_port );
  }

  ConnectionError ConnBoshMultStat::connect()
  {
  ...
  void ConnBoshMultStat::disconnect()
  {
  ...
  ConnectionError ConnBoshMultStat::recv( int timeout )
  {
    if( m_state == StateDisconnected )
      return ConnNotConnected;
    ...
    {
      m_logInstance.dbg( LogAreaClassConnectionBOSH,
                         "Sending empty request (or there is data in the send buffer)"+m_sendBuffer );
      sendHttpbindXML();
    }
  ...
  bool ConnBoshMultStat::send( const std::string& data )
  {
...
//       }
  ...
  bool ConnBoshMultStat::sendHttpbindXML()
  {
  ...
  bool ConnBoshMultStat::sendRequest( const std::string& xml )
  {
...
//       printf( "Error while trying to send on socket (state: %d)\n", conn->state() );
  ...
  ConnectionError ConnBoshMultStat::receive()
  {
  ...
  void ConnBoshMultStat::getStatistics( long int& totalIn, long int& totalOut )
  {
...
//      requestBody.addAttribute( "xmpp:version", "1.0" );
  ...
  void ConnBoshMultStat::handleDisconnect( const ConnectionBase* /*connection*/,
...
//         printf( "A TCP connection %p was disconnected (reason: %d).\n", connection, reason );
  ...
  void ConnBoshMultStat::handleTag( Tag* tag )
  {
  ...
  void ConnBoshMultStat::putConnection()
  {
