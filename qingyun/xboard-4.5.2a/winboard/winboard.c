...
{ DLG_Challenge, IDC_Event, OPT_NoSound, OPT_DefaultBeep, OPT_BuiltInSound,
  OPT_WavFile, OPT_BrowseSound, OPT_DefaultSounds, IDOK, IDCANCEL, OPT_PlaySound }, 
...
WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_COMMAND: /* message: command from application menu */
    ...
    case IDD_CHALLENGE:
      ChallengeOptionsPopup(hwnd,lParam);
      break;
    case EP_WhitePawn:
  ...
  case WM_USER_Input:
    ...
    break;
  case WM_BOSH_Input:
    BoshInput(hwnd, message, wParam, lParam);
    break;
  case WM_SUBSCRIBER_INPUT:
    SubscriberInput(hwnd, message, wParam, lParam);
    break;
...
ConsoleInputSubclass(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_CHAR:
	...
	ConsoleOutput("\n", 1, TRUE);
      ...
      SendMessage(hwndMain, WM_SUBSCRIBER_INPUT, 0, (LPARAM) is);
      /* fall thru */
    case '\033': /* Escape key */
      SetWindowText(hwnd, "");
...
DWORD
SocketInputThreadDummy(LPVOID arg)
{
  InputSource *is;

  is = (InputSource *) arg;
  if (is->hThread != NULL) {
    bosh_main(0,(void **)NULL);
  }
  return 0;
}
...
VOID
BoshInput(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    int i = (int)lParam;
    if(i == REQUEST_USERNAME)
    {
        char tmp[MSG_SIZ];
        snprintf(tmp, MSG_SIZ, _("please input your username:"));
        SendToPlayer(tmp, strlen(tmp));
    }
    else if( i == REQUEST_PASSWORD)
    {
        char tmp[MSG_SIZ];
        snprintf(tmp, MSG_SIZ, _("please input your password:"));
        SendToPlayer(tmp, strlen(tmp));
        EchoOff();
    }
    else if( i == ACK_HAVE_LOGGEDIN)
    {
        char tmp[MSG_SIZ];
        snprintf(tmp, MSG_SIZ, _("now you've logged in."));
        set_loggedOn(TRUE);
        SendToPlayer(tmp, strlen(tmp));
        send_xmpp_request(SEND_SEARCH_USERS , NULL);//send GetMyProfile message.
    }
    else if( i == REQUEST_UPDATE_SEEKGRAPH)
    {
        draw_seekgraph();
    }
    else if( i == REQUEST_FRONTEND_START_CHALLENGE)
    {
        char *p = (char *)wParam;
        if(strcmp(p,"white") == 0)
        {
    ...
    else if( i == REQUEST_FRONTEND_MOVED)
    {
         /*ParseBoard12((char *)wParam);*/
/*         set_ics_user_moved(0); */
    }
    else if( i == ACK_TO_CONNECT)
    {
        char tmp[MSG_SIZ];
        snprintf(tmp, MSG_SIZ, _("Connecting server...\n"));
        SendToPlayer(tmp, strlen(tmp));

    }
    else if( i == ACK_EXIT_CONNECT)
    {
        char tmp[MSG_SIZ];
        snprintf(tmp, MSG_SIZ, _("\nConnection to the server terminated.\n"));
        SendToPlayer(tmp, strlen(tmp));
...
VOID
SubscriberInput(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  InputSource *is;
...
VOID setup_a_challenge(char *opposite)
{
  int n = 0, i = 0;
  for(; i<strlen(opposite);i++)
  {
    if(opposite[i] == ' ')
    {
        n = i-1;
  ...
  if(n>=0)
  {
    char *new_opp = (char *)malloc(n+2);
    strncpy(new_opp,opposite,n+1);
    new_opp[n+1]=0;
    /*free the memory after recv message*/
    SendMessage(hwndMain, WM_COMMAND, MAKEWPARAM(IDD_CHALLENGE,0), (LPARAM)new_opp);
  }
  else
  {
  //do nothing
  ...
  if (pr == NoProc) {
    ...
    */
    ...
    case CPSock:
      is->sock = cp->sock;
      is->hThread =
	CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) SocketInputThreadDummy,
		     (LPVOID) is, CREATE_SUSPENDED, &is->id);
      break;
...
#endif
    outCount = send_bosh_message(message,count);
