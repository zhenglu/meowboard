...
char *builtinGameCategories[] = BUILT_IN_GAME_CATEGORY;
char *builtinGameExtSec[] = BUILT_IN_GAME_EXTSEC;
char *builtinTC_Britz[] = BUILT_IN_TC_BRITZ;
char *builtinTC_Quick[] = BUILT_IN_TC_QUICK;
char *builtinTC_Normal[] = BUILT_IN_TC_NORMAL;
char *builtinTC_Unlimited[] = BUILT_IN_TC_UNLIMITED;
...
LRESULT CALLBACK
ChallengeOptionsDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  static HWND hBCat,hBSec,hTC;
  DWORD index;
  char *opposite;
  ...
  case WM_INITDIALOG:
    /* Center the dialog over the application window */
    CenterWindow (hDlg, GetWindow (hDlg, GW_OWNER));
    Translate(hDlg, DLG_Challenge);

    /* Initialize the built-in category */
    hBCat = GetDlgItem(hDlg, CBO_CHAL_Category);
    InitComboStrings(hBCat, builtinGameCategories);

    hBSec = GetDlgItem(hDlg, CBO_CHAL_ExtSeconds);
    InitComboStrings(hBSec, builtinGameExtSec);

    hTC = GetDlgItem(hDlg, CBO_Challenge_TimeControl);

//default values
    InitComboStrings(hTC, builtinTC_Normal);

    CheckRadioButton(hDlg, OPT_MYBLACK, OPT_MYRANDOM, OPT_MYRANDOM);
//default category: normal
    SendMessage(hBCat, CB_SETCURSEL, (WPARAM) 2, (LPARAM) 0);
//default extra seconds:0second
    SendMessage(hBSec, CB_SETCURSEL, (WPARAM) 0, (LPARAM) 0);
//default TC
    SendMessage(hTC, CB_SETCURSEL, (WPARAM) 0, (LPARAM) 0);

    opposite = (char *)lParam;
    if(opposite)
    {
        SetDlgItemText(hDlg, EDT_OppositePlayer, opposite);
  ...
  case WM_COMMAND: /* message: received a command */
    ...
    if (((HWND)lParam == hBCat) &&
	(HIWORD(wParam) == CBN_SELCHANGE)) {
      /* now get the settings for the newly selected event */
      index = SendMessage(hBCat, CB_GETCURSEL, (WPARAM)0, (LPARAM)0);
      if(index == 0)
          InitComboStrings(hTC, builtinTC_Quick);
      else if(index == 1)
          InitComboStrings(hTC, builtinTC_Britz);
      else if(index == 2)
          InitComboStrings(hTC, builtinTC_Normal);
      else if(index == 3)
          InitComboStrings(hTC, builtinTC_Unlimited);
    ...
    case IDOK:
      /*
       * save the name for the currently selected event
       */
      /* send message to xmpp server*/
      {
        char Oponent[MSG_SIZ]={0};
        char Color[MSG_SIZ]={0};
        char TimeStr[MSG_SIZ]={0};
        char Category[MSG_SIZ]={0};
        char IncStr[MSG_SIZ]={0};
        GetDlgItemText(hDlg, EDT_OppositePlayer, Oponent, MSG_SIZ);
        if (IsDlgButtonChecked(hDlg, OPT_MYBLACK))
            strcpy(Color,"black");
        else if(IsDlgButtonChecked(hDlg, OPT_MYRED))
            strcpy(Color,"white");

        GetDlgItemText(hDlg, CBO_Challenge_TimeControl, TimeStr, MSG_SIZ);
        GetDlgItemText(hDlg, CBO_CHAL_Category, Category, MSG_SIZ);
        GetDlgItemText(hDlg, CBO_CHAL_ExtSeconds, IncStr, MSG_SIZ);
        int Rated = TRUE;
        send_xmpp_challenge_message(Oponent, Color, TimeStr, IncStr, Category, Rated);
      }
      EndDialog(hDlg, TRUE);
      return TRUE;
...
VOID ChallengeOptionsPopup(HWND hwnd, LPARAM lParam)
{
  FARPROC lpProc;

  lpProc = MakeProcInstance((FARPROC)ChallengeOptionsDialog, hInst);
  DialogBoxParam(hInst, MAKEINTRESOURCE(DLG_Challenge),
    hwnd, (DLGPROC)lpProc, lParam);
